...
多数客户端只需要 API 密钥、接口地址和模型名称。请先按 [新版控制台流程](/guide/console) 创建并复制 API 密钥，再从模型广场复制实际可用的模型名称。
...
::: tip 排查顺序
报 401 时检查 API 密钥；报 404 时检查地址是否出现 `/v1/v1` 或所选模型是否支持当前协议；报模型不存在时，从模型广场重新复制模型名称。请求已经到达平台时，可在 [使用日志](https://www.fastapi.cool/usage-logs/common) 查看详细错误。
:::
