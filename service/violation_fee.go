...
func HasCSAMViolationMarker(err *types.FastAPIError) bool {
	if err == nil {
		return false
	}
...
func WrapAsViolationFeeGrokCSAM(err *types.FastAPIError) *types.FastAPIError {
	if err == nil {
		return nil
	}
...
func NormalizeViolationFeeError(err *types.FastAPIError) *types.FastAPIError {
	if err == nil {
		return nil
	}
...
func shouldChargeViolationFee(err *types.FastAPIError) bool {
	if err == nil {
		return false
	}
...
func ChargeViolationFeeIfNeeded(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, apiErr *types.FastAPIError) bool {
