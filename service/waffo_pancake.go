...
// are exposed separately for self-service flows driven from fast-api's own UI.
...
func WaffoPancakeBuyerIdentityFromUserID(userID int) string {
	return fmt.Sprintf("fast-api-user-%d", userID)
}
...
const (
	defaultWaffoPancakeStoreName   = "fast-api-store"
	defaultWaffoPancakeProductName = "fast-api-charge-product"
)
...
//
// OnetimeProduct (not SubscriptionProduct) because fast-api has no renewal-
// event handling; Pancake auto-renewing without fast-api extending user
